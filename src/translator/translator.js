...
Hooks.once("init", () => {
    game.langPtPf2e = Translator.get();

    // Register token setting
    game.settings.register("lang-pt-BR-pf2e", "token", {
        name: "Imagem de retrato como token",
        hint: "Ao importar um NPC traduzido de um compêndio, a imagem do retrato deve ser usada como símbolo em vez da imagem regular do símbolo?",
        scope: "world",
        type: Boolean,
        config: true,
        default: false,
...
class Translator {
    ...
    async initialize() {
        ...
        const config = await Promise.all([
            fetch("modules/lang-pt-BR-pf2e/src/translator/translator-config.json")
                .then((r) => r.json())
                .catch((_e) => {
                    console.error("lang-pt-BR-pf2e: Arquivo de configuração do tradutor não encontrado.");
                }),
        ]);
        ...
        if (dictionaryPath) {
            ...
            console.error("lang-pt-BR-pf2e: Dicionário não disponível");
        }
        ...
        Hooks.callAll("langPtPf2e.ready");
    }
    ...
    // Register a media path for a compendium containing portrait and token images
    addMediaPath(compendium, path) {
        ["portraits", "tokens"].forEach(async (imageType) => {
            const imagePath = game.settings.get("lang-pt-BR-pf2e", "token")
    ...
    translateDualLanguage(data, translation) {
        if (!translation || data === translation) {
            return data;
        } else if (game.settings.get("lang-pt-BR-pf2e", "dual-language-names")) {
